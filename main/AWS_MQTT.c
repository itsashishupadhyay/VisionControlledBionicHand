...
QueueHandle_t xPushMessageQueue = NULL;
                                    ...
                                    IoT_Publish_Message_Params *params, void *pData)
{
    ...
    // ((int)params->payloadLen%2 == 0) ?OpenFingers():CloseFingers();
    int i, GetFinger, GetPowerPercent, len = (int)params->payloadLen;
    struct json_token t;
    FingerActivation SendTaskQueue;
    for (i = 0; json_scanf_array_elem((char *)params->payload, len, ".Data", i, &t) > 0; i++)
    {
        json_scanf(t.ptr, t.len, "{finger: %d,PowerPercent: %d}", &GetFinger, &GetPowerPercent);
        printf("{finger: %d,PowerPercent: %d}", GetFinger, GetPowerPercent);
...
void aws_iot_task(void *param)
{
    ...
    rc = aws_iot_mqtt_subscribe(&client, CONFIG_AWS_TOPIC_SUB, strlen(CONFIG_AWS_TOPIC_SUB), QOS0, iot_subscribe_callback_handler, NULL);
    if (SUCCESS != rc)
    {
    ...
    while ((NETWORK_ATTEMPTING_RECONNECT == rc || NETWORK_RECONNECTED == rc || SUCCESS == rc))
    {
        ...
        rc = aws_iot_mqtt_publish(&client, CONFIG_AWS_TOPIC_PUB, strlen(CONFIG_AWS_TOPIC_PUB), &paramsQOS0);

        sprintf(cPayload, "%s : %d ", "hello from ESP32 (QOS1)", i++);
        paramsQOS1.payloadLen = strlen(cPayload);
        rc = aws_iot_mqtt_publish(&client,CONFIG_AWS_TOPIC_PUB, strlen(CONFIG_AWS_TOPIC_PUB), &paramsQOS1);*/

        if (xPushMessageQueue != NULL)
        {
            if (xQueueReceive(xPushMessageQueue,
                              &(CurrentTaskQueue),
...
void aws_sub_pub_app_main()
{
    ...
    initialise_wifi();
    xPushMessageQueue = xQueueCreate(10, sizeof(FingerActivation));
    if (xPushMessageQueue == NULL)
    {
        ESP_LOGE(TAG,"The Queue Creation Failed");
    }
    xTaskCreatePinnedToCore(&aws_iot_task, "aws_iot_task", 9216, NULL, 0, NULL, 1);
}
